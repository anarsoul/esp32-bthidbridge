...
static esp_hid_bt_acl_conn_cb_t s_bt_acl_conn_cb = NULL;
...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    /* Fall back to incomplete service list — many HID devices use this type */
    if (uuid == 0) {
    ...
    ESP_LOGD(TAG, "BLE: " ESP_BD_ADDR_STR " UUID:0x%04x APPEARANCE:0x%04x RSSI:%d NAME:%s",
             ESP_BD_ADDR_HEX(scan_rst->bda), uuid, appearance, scan_rst->rssi,
             adv_name_len ? name : "");

    /* Accept devices that advertise HID service UUID or a HID appearance value */
    bool is_hid_svc = (uuid == ESP_GATT_UUID_HID_SVC);
    bool is_hid_appearance = (appearance >= ESP_HID_APPEARANCE_GENERIC &&
                               appearance <= ESP_HID_APPEARANCE_GAMEPAD);
    if (is_hid_svc || is_hid_appearance) {
...
static void bt_gap_event_handler(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    case ESP_BT_GAP_ACL_CONN_CMPL_STAT_EVT:
        if (param->acl_conn_cmpl_stat.stat == ESP_BT_STATUS_SUCCESS && s_bt_acl_conn_cb) {
            s_bt_acl_conn_cb(param->acl_conn_cmpl_stat.bda);
        }
        break;
...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
        ESP_LOGI(TAG, "BLE conn params updated: interval=%ums latency=%u timeout=%ums status=%d",
                 param->update_conn_params.conn_int * 5 / 4,  /* units of 1.25ms */
...
#if CONFIG_BT_BLE_ENABLED
esp_err_t esp_hid_ble_scan(uint32_t seconds, size_t *num_results, esp_hid_scan_result_t **results)
{
    if (num_ble_scan_results || ble_scan_results) {
        ESP_LOGE(TAG, "There are old BLE scan results. Free them first!");
        return ESP_FAIL;
    }
